...
module.exports = (app) => {
	...
	app.all("/fortnite/api/matchmaking/session/:sessionKey/join", (req, res) => {
		res.json([])
	});
	//matchmaking xmpp
    ...
    app.get('/stats/level/:accountId/:level', (req, res) => {
		...
		fs.readFile(`./config/${accountId}/profiles/profile_athena.json`, 'utf-8', (err, jsonString) => {
		  if (err) {
			...
			const data = JSON.parse(jsonString);
			data.items.victoryCrown.attributes.victory_crown_account_data.total_royal_royales_achieved_count = crowns;
		...
		fs.readFile(`./config/${accountId}/profiles/profile_common_core.json`, 'utf-8', (err, jsonString) => {
		  if (err) {
			...
			const data = JSON.parse(jsonString);
			data.items.Currency.quantity = vbucks;
